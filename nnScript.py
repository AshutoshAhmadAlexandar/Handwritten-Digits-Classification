...
def nnPredict(w1, w2, data):
    ...
    for t in data:
        ...
        h = sigmoid(np.dot(w1,inputBias))
        
        hBias=np.hstack((h,np.ones(1)))  #size (d+1)*1
        output = sigmoid(np.dot(w2,hBias))
...
#Collecting all the values after preprocess so that we can dump those on params pickle file
